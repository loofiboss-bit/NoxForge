...
class ReleaseWorkflowTests(unittest.TestCase):
    ...
    def test_release_metadata_is_selected_from_the_manifest_and_tag(self) -> None:
        ...
        self.assertIn('gh release edit "$NOXFORGE_RELEASE_REF" --draft=false', self.workflow)
        self.assertIn('gh release delete "$NOXFORGE_RELEASE_REF" --yes', self.workflow)
        self.assertIn('gh release delete "$staging_ref" --yes', self.workflow)
        ...
        self.assertIn('staging_ref:', self.workflow)
        self.assertIn('NOXFORGE_STAGING_REF: ${{ inputs.staging_ref }}', self.workflow)
        self.assertIn('NOXFORGE_RELEASE_REF: ${{ inputs.ref }}', self.workflow)
        self.assertIn('staging_ref="$NOXFORGE_STAGING_REF"', self.workflow)
        self.assertNotIn('staging_ref="${{ inputs.staging_ref }}"', self.workflow)
        self.assertIn("safe replacement requires a pre-created staging_ref input", self.workflow)
        self.assertIn("staging_ref must differ from the release ref", self.workflow)
        self.assertIn('git ls-remote --exit-code origin "refs/tags/$staging_ref"', self.workflow)
        self.assertIn('staging tag does not exist:', self.workflow)
        self.assertIn('git fetch origin "refs/tags/$staging_ref:refs/tags/$staging_ref"', self.workflow)
        self.assertIn('refs/tags/$NOXFORGE_RELEASE_REF^{commit}', self.workflow)
        self.assertIn('refs/tags/$staging_ref^{commit}', self.workflow)
